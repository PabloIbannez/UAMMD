...
namespace uammd{
  namespace Hydro{
	...
	real3 DivNoise = computeNoiseDivergence(cell, icell,
						gridVels,
						grid,
						noiseAmp,
						random);
	real3 Lv = computeVelLaplacian(cell, icell,
				       gridVels,
				       grid);
	real3 advection = computeAdvection(cell, icell,
					   gridVels,
					   grid, density);
	...
	gridVelsPrediction.resize(3*ncells);
    ...
    void ICM::initFluid(){
      int ncells = grid.getNumberCells();
      thrust::host_vector<cufftComplex> cellVelocityCPU = gridVels;
    ...
    void ICM::unperturbedFluidForcing(){
      ...
      real noiseAmp = sqrt(2*temperature*viscosity/(dV*dt));
      int ncells = grid.getNumberCells();
    ...
    void ICM::applyStokesSolutionOperator(){
      ...
      cufftReal* d_gridForcing = (cufftReal*)thrust::raw_pointer_cast(gridVelsPrediction.data());
      cufftComplex* d_gridVels = (cufftComplex*)thrust::raw_pointer_cast(gridVelsPrediction.data());
	...
	sys->log<System::CRITICAL>("[Hydro::ICM] Error in forward CUFFT");
      }
      ...
      ICM_ns::solveStokesFourier<<<NblocksCells, NthreadsCells>>>((cufftComplex3*)d_gridForcing,
	...
	cufftExecComplex2Real<real>(cufft_plan_inverse,
      ...
      if(cufftStatus != CUFFT_SUCCESS){
	sys->log<System::CRITICAL>("[Hydro::ICM] Error in inverse CUFFT");
      }
    }
    ...
    void ICM::predictorStep(){
      ...
      dim3 NthreadsCells = dim3(BLOCKSIZE);
      dim3 NblocksCells;
      ...
      {
	auto force = pd->getForce(access::location::gpu, access::mode::write);     
	fillWithGPU<<<Nblocks, Nthreads>>>(force.raw(), make_real4(0), numberParticles);     
      }
